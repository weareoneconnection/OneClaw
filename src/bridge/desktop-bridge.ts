...
function checkArtifactsDir(config: AppConfig): BridgeCheck {
  try {
...
function checkSystemEvents(): BridgeCheck {
...
function checkScreenCapture(): BridgeCheck {
  if (os.platform() !== "darwin") {
    return {
...
export function getBridgeDiagnostics(config: AppConfig): BridgeCheck[] {
  return [
    {
      key: "mode",
      label: "Bridge mode",
...
export function getBridgeStatus(config: AppConfig) {
...
export function getBridgeRegistration(config: AppConfig, capabilities: BridgeCapability[]) {
