...
export async function bootstrap(options?: { workerOnly?: boolean }) {
  ...
  const regs = [
    { action: "browser.open", workerName: "browser_worker", risk: "low", description: "Open a browser target" },
    { action: "browser.click", workerName: "browser_worker", risk: "low", description: "Click a browser target" },
    { action: "browser.type", workerName: "browser_worker", risk: "medium", description: "Type into a browser field" },
    { action: "browser.screenshot", workerName: "browser_worker", risk: "low", description: "Take a screenshot" },
    { action: "browser.scrape", workerName: "browser_worker", risk: "low", description: "Scrape text or html" },
    { action: "message.send", workerName: "messaging_worker", risk: "medium", description: "Send a message" },
    { action: "api.request", workerName: "api_worker", risk: "medium", description: "Perform an HTTP request" },
    { action: "file.read", workerName: "file_worker", risk: "low", description: "Read a file" },
    { action: "file.write", workerName: "file_worker", risk: "medium", description: "Write a file" },
    { action: "social.post", workerName: "social_worker", risk: "high", description: "Publish social content" },
    { action: "chain.query", workerName: "web3_worker", risk: "low", description: "Query chain state" },
    { action: "wallet.read", workerName: "web3_worker", risk: "medium", description: "Read wallet data" },
  ] as const;

  for (const registration of regs) capabilities.register(registration);

  const runtime = new ExecutionRuntime(capabilities, workers, policy, taskStore, sessionManager);
  const queue = await createTaskQueue({ config, planner, runtime, taskStore });
