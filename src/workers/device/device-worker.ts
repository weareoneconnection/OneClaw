...
export class DeviceWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "device.status.read") {
      if (!deviceId) return { ok: false, error: "device.status.read requires input.deviceId" };
    ...
    if (context.action === "device.command.prepare") {
      ...
      if (!deviceId || !command) return { ok: false, error: "device.command.prepare requires input.deviceId and input.command" };
    ...
    if (context.action === "iot.sensor.read") {
      ...
      if (!sensorId) return { ok: false, error: "iot.sensor.read requires input.sensorId" };
    ...
    if (context.action === "robot.task.prepare") {
      ...
      if (!task) return { ok: false, error: "robot.task.prepare requires input.task" };
