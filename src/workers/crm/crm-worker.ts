...
export class CrmWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "crm.lead.create") {
      ...
      if (!name) return { ok: false, error: "crm.lead.create requires input.name" };
    ...
    if (context.action === "crm.contact.update") {
      ...
      if (!contactId) return { ok: false, error: "crm.contact.update requires input.contactId" };
    ...
    if (context.action === "crm.deal.create") {
      ...
      if (!title) return { ok: false, error: "crm.deal.create requires input.title" };
    ...
    if (context.action === "crm.activity.log") {
      ...
      if (!subject) return { ok: false, error: "crm.activity.log requires input.subject" };
