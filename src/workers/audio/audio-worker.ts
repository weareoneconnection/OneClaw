...
export class AudioWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "audio.transcribe") {
      if (!path) return { ok: false, error: "audio.transcribe requires input.path" };
    ...
    if (context.action === "audio.synthesize") {
      ...
      if (!text) return { ok: false, error: "audio.synthesize requires input.text" };
    ...
    if (context.action === "voice.command.parse") {
      ...
      if (!text) return { ok: false, error: "voice.command.parse requires input.text or input.transcript" };
