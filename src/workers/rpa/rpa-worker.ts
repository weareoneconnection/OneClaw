...
function isAllowedApp(app: string, allowlist: string[]) {
...
function resolveApplicationPath(app: string): string | undefined {
...
function modifierFor(key: string): string | undefined {
  const normalized = key.toLowerCase();
...
export class RpaWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "desktop.click") {
      ...
      if (x === undefined || y === undefined) return { ok: false, error: "desktop.click requires numeric input.x and input.y" };
    ...
    if (context.action === "desktop.hotkey") {
      ...
      if (!keys.length) return { ok: false, error: "desktop.hotkey requires input.keys" };

      const modifiers = keys.map(modifierFor).filter((item): item is string => Boolean(item));
      const mainKeys = keys.filter((key) => !modifierFor(key));
      const mainKey = mainKeys[mainKeys.length - 1];
      if (!mainKey) return { ok: false, error: "desktop.hotkey requires one non-modifier key" };
