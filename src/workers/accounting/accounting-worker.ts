...
export class AccountingWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "finance.invoice.parse") {
      ...
      if (!path) return { ok: false, error: "finance.invoice.parse requires input.path" };
    ...
    if (context.action === "finance.reconcile") {
      ...
      if (!source) return { ok: false, error: "finance.reconcile requires input.source" };
    ...
    if (context.action === "finance.budget.review") {
      ...
      if (!budgetId) return { ok: false, error: "finance.budget.review requires input.budgetId or input.projectId" };
