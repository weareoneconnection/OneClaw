...
export class VisionWorker implements Worker {
  ...
  async execute(input: Record<string, Json>, context: ExecutionContext): Promise<WorkerExecutionResult> {
    ...
    if (context.action === "image.analyze") {
      if (!path) return { ok: false, error: "image.analyze requires input.path" };
    ...
    if (context.action === "image.extractText") {
      if (!path) return { ok: false, error: "image.extractText requires input.path" };
    ...
    if (context.action === "construction.photo.inspect") {
      if (!path) return { ok: false, error: "construction.photo.inspect requires input.path" };
