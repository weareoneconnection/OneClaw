...
type GraphLayerOptions = {
  sortEachLayer?: boolean;
};

export class TaskGraph {
  private readonly task: NormalizedTaskDefinition;
  private readonly stepMap = new Map<string, GraphStep>();
  ...
  private build(): void {
    ...
    for (const [stepId, node] of this.nodes.entries()) {
      for (const depId of node.dependsOn) {
        ...
        if (!depNode) {
          throw new Error(
            `Step "${stepId}" (${String(node.step.action)}) depends on missing step "${depId}"`,
          );
        }
  ...
  private normalizeRequiredStepId(value: unknown): string {
  ...
  private normalizeOptionalStepId(value: unknown): string | undefined {
    const id = String(value ?? "").trim();
    return id || undefined;
  }
  ...
  private buildTopoLayers(options?: GraphLayerOptions): string[][] {
    ...
    if (visitedCount !== this.nodes.size) {
      throw new Error(
        "Task graph layering failed: unresolved cycle or graph corruption",
      );
    }
  ...
  getRunnableSteps(
    ...
    runningStepIds?: Iterable<string>,
  ): GraphStep[] {
    const completed = new Set(completedStepIds);
    const running = new Set(runningStepIds ?? []);
    const runnable: GraphStep[] = [];
    ...
    for (const stepId of this.getStableStepIds()) {
      ...
      if (running.has(stepId)) continue;
  ...
  isRunnable(
    ...
    if (completed.has(stepId)) return false;
  ...
  isBlocked(
    ...
    if (completed.has(stepId)) return false;
    if (this.isRunnable(stepId, completed, runningStepIds)) return false;
  ...
  getBlockedSteps(
    ...
    const blocked: GraphStep[] = [];

    for (const stepId of this.getStableStepIds()) {
      if (
        this.isBlocked(stepId, completedStepIds, failedStepIds, runningStepIds)
      ) {
  ...
  getExecutionBatches(): GraphStep[][] {
    return this.buildTopoLayers({ sortEachLayer: true }).map((layer) =>
      layer.map((stepId) => this.get(stepId)),
    );
  }
