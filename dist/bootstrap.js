...
function requiredInputForAction(action) {
    ...
    if (action === "message.send")
        return ["text"];
    if (action.startsWith("x.search"))
        return ["query"];
    if (action.startsWith("construction."))
        return ["title"];
    if (action === "web3.transfer")
        return ["chain", "to", "amount"];
    ...
    if (action === "calendar.event.update")
        return ["eventId"];
    if (action === "document.parse")
        return ["path"];
    if (action === "document.generate")
        return ["path", "content"];
    if (action === "spreadsheet.read")
        return ["path"];
    if (action === "spreadsheet.write")
        return ["path", "rows"];
    if (action === "database.query" || action === "database.write")
        return ["sql"];
    ...
    if (action === "identity.resolve")
        return ["subject"];
    if (action === "secret.check")
        return ["key"];
    if (action === "crm.lead.create")
        return ["name"];
    if (action === "crm.contact.update")
        return ["contactId"];
    if (action === "crm.deal.create")
        return ["title"];
    if (action === "crm.activity.log")
        return ["subject"];
    if (action === "commerce.product.search")
        return ["query"];
    if (action === "commerce.order.prepare")
        return ["item"];
    if (action === "payment.invoice.create")
        return ["customer"];
    if (action === "payment.charge.prepare")
        return ["amount"];
    if (action === "git.issue.create")
        return ["repo", "title"];
    if (action === "git.pr.create")
    ...
    if (action === "iot.sensor.read")
        return ["sensorId"];
    if (action === "robot.task.prepare")
        return ["task"];
    if (action === "knowledge.upsert")
        return ["id", "text"];
    if (action === "knowledge.query")
        return ["query"];
    if (action === "vector.upsert")
        return ["id", "vector"];
    if (action === "audio.transcribe")
        return ["path"];
    if (action === "audio.synthesize")
        return ["text"];
    if (action === "voice.command.parse")
        return ["text"];
    if (action.startsWith("image."))
        return ["path"];
    if (action === "construction.photo.inspect")
        return ["path"];
    if (action === "video.analyze" || action === "video.summarize")
        return ["path"];
    if (action === "camera.stream.inspect")
        return ["streamUrl"];
    if (action === "geo.geocode")
        return ["address"];
    if (action === "geo.route.plan")
    ...
    if (action.startsWith("legal.contract.") || action === "legal.risk.review")
        return [];
    if (action === "legal.approval.package")
        return ["title"];
    if (action === "finance.invoice.parse")
        return ["path"];
    if (action === "finance.reconcile")
        return ["source"];
    if (action === "finance.budget.review")
        return ["budgetId"];
    if (action.startsWith("simulation.") || action.startsWith("digitalTwin."))
        return ["projectId"];
    return [];
}
function outputContractForAction(action) {
    if (action.startsWith("browser."))
        return ["status", "url", "artifacts"];
    if (action.startsWith("api."))
    ...
    if (action.startsWith("document."))
        return ["path", "text", "artifacts"];
    if (action.startsWith("spreadsheet."))
        return ["path", "rows", "artifacts"];
    if (action.startsWith("database."))
        return ["rows", "rowCount", "status"];
    if (action.startsWith("search."))
    ...
    if (action.startsWith("identity.") || action.startsWith("permission.") || action.startsWith("secret."))
        return ["status", "configured"];
    if (action.startsWith("shell."))
        return ["stdout", "stderr"];
    if (action.startsWith("crm."))
        return ["status", "payload"];
    if (action.startsWith("commerce.") || action.startsWith("payment."))
    ...
    if (action.startsWith("knowledge.") || action.startsWith("vector."))
        return ["status", "namespace", "results"];
    if (action.startsWith("audio.") || action.startsWith("voice."))
        return ["status", "transcript", "text"];
    if (action.startsWith("image.") || action === "construction.photo.inspect")
        return ["status", "findings", "text"];
    if (action.startsWith("video.") || action.startsWith("camera."))
    ...
    if (action.startsWith("finance."))
        return ["status", "fields", "exceptions", "variances"];
    if (action.startsWith("simulation.") || action.startsWith("digitalTwin."))
        return ["status", "scenarios", "forecast"];
    return ["status"];
}
function permissionsForAction(action) {
    if (action.startsWith("browser."))
        return ["browser"];
    if (action.startsWith("api."))
        return ["network"];
    if (action.startsWith("file."))
        return ["filesystem"];
    if (action === "social.post" || action.startsWith("message."))
        return ["communication"];
    if (action.startsWith("web3."))
        return ["transaction"];
    if (action.startsWith("email."))
        return ["email"];
    if (action.startsWith("calendar."))
        return ["calendar"];
    if (action.startsWith("document.") || action.startsWith("spreadsheet.") || action.startsWith("storage."))
        return ["filesystem"];
    if (action.startsWith("database."))
        return ["database"];
    if (action.startsWith("search."))
    ...
    if (action.startsWith("audio.") || action.startsWith("voice."))
        return ["audio"];
    if (action.startsWith("image.") || action.startsWith("video.") || action.startsWith("camera.") || action === "construction.photo.inspect")
        return ["vision"];
    if (action.startsWith("geo."))
        return ["geo"];
    if (action.startsWith("desktop."))
...
function maturityForAction(action) {
    if (action === "social.post" || action.startsWith("x."))
        return "production";
    if (action.startsWith("document.") || action.startsWith("spreadsheet.") || action.startsWith("storage.") || action.startsWith("identity.") || action.startsWith("secret."))
        return "guarded";
    ...
    if (action.startsWith("audio.") || action.startsWith("voice.") || action.startsWith("image.") || action.startsWith("video.") || action.startsWith("camera.") || action.startsWith("geo.") || action.startsWith("desktop.") || action.startsWith("legal.") || action.startsWith("finance.") || action.startsWith("simulation.") || action.startsWith("digitalTwin."))
        return "guarded";
    if (action.startsWith("browser.") || action.startsWith("api.") || action.startsWith("file.") || action.startsWith("message.") || action.startsWith("human."))
        return "guarded";
    if (action.startsWith("construction."))
        return "planned";
    if (["web3.contract.write", "web3.transfer"].includes(action))
        return "stub";
    return "guarded";
}
function liveModeForAction(action, maturity) {
    if (maturity === "stub")
        return "disabled";
    if (maturity === "planned")
        return "prepared";
...
function connectorKeyForAction(action) {
    if (action.startsWith("x.") || action === "social.post")
        return "x";
    ...
    if (action.startsWith("api."))
        return "api";
    if (action.startsWith("file.") || action.startsWith("document.") || action.startsWith("spreadsheet.") || action.startsWith("storage."))
        return "filesystem";
    if (action.startsWith("database."))
        return "database";
    if (action.startsWith("shell."))
...
export async function bootstrap(options) {
    ...
    if (!options?.workerOnly && config.queueMode === "bullmq") {
        console.log("BullMQ mode enabled. Start a separate worker with: npm run dev:worker");
    }
    return {
        config,
        taskStore,
