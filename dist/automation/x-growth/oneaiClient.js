export async function runOneAIWorkflow(payload) {
    let baseUrl = process.env.ONEAI_API_BASE_URL ??
    ...
    const adminKey = process.env.ONEAI_ADMIN_API_KEY ??
        process.env.ONEAI_ADMIN_KEY ??
        process.env.ONEAI_API_KEY ??
        "";
    if (!adminKey) {
        throw new Error("Missing OneAI admin key: set ONEAI_ADMIN_API_KEY, ONEAI_ADMIN_KEY, or ONEAI_API_KEY");
    }
    const headers = {
