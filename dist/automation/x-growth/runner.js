...
export class XGrowthRunner {
    ...
    async runEngage() {
        ...
        const freshCandidates = candidateTweets.filter((tweet) => !state.seenReplyTweetIds.includes(tweet.tweetId));
        ...
        const workflowResult = await runOneAIWorkflow({
            task: "x_engage",
            input: {
                message: "Review these candidate tweets and reply when the interaction creates real credibility, useful visibility, or builder-grade positioning for OneAI.",
                lang: "en",
