...
export class LegalWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "legal.contract.extract") {
            if (!text && !path)
                return { ok: false, error: "legal.contract.extract requires input.text or input.path" };
        ...
        if (context.action === "legal.risk.review") {
            if (!text && !path)
                return { ok: false, error: "legal.risk.review requires input.text or input.path" };
        ...
        if (context.action === "legal.approval.package") {
            ...
            if (!title)
                return { ok: false, error: "legal.approval.package requires input.title" };
