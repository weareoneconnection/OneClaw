...
export class EmailWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "email.draft") {
            if (!subject || !body)
                return { ok: false, error: "email.draft requires input.subject and input.body" };
        ...
        if (context.action === "email.send") {
            if (!to.length || !subject || !body)
                return { ok: false, error: "email.send requires input.to, input.subject, and input.body" };
            return {
                ok: true,
                output: {
                    provider: "email",
        ...
        if (context.action === "email.search") {
            ...
            if (!query)
                return { ok: false, error: "email.search requires input.query" };
