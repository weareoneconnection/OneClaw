        ...
        try {
            switch (context.action) {
                ...
                case "browser.type": {
                    ...
                    await context.log(`BrowserWorker typed selector=${selector} textLength=${text.length}`);
                    return {
                        ok: true,
                        output: {
                            action: context.action,
                ...
                case "browser.screenshot": {
                    ...
                    const filePath = await this.browserAdapter.screenshot(session.page, context.taskId, context.stepId, requestedPath, fullPage);
                    const currentUrl = session.page.url();
                    const title = await session.page.title();
                ...
                case "browser.scrape": {
                    ...
                    const currentUrl = session.page.url();
                    const title = await session.page.title();
                    await context.log(`BrowserWorker scraped mode=${mode} selector=${selector ?? "body"} contentLength=${content.length}`);
                    return {
                        ok: true,
                        output: {
                            action: context.action,
                ...
                case "browser.extract": {
                    ...
                    await context.log(`BrowserWorker extracted url=${extracted.url} title=${extracted.title} textLength=${extracted.text.length}`);
                    return {
                        ok: true,
                        output: {
                            action: context.action,
