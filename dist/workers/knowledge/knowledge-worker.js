...
export class KnowledgeWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "knowledge.upsert") {
            ...
            if (!id || !text)
                return { ok: false, error: "knowledge.upsert requires input.id and input.text" };
        ...
        if (context.action === "knowledge.query") {
            ...
            if (!query)
                return { ok: false, error: "knowledge.query requires input.query" };
        ...
        if (context.action === "vector.upsert") {
            ...
            if (!id || !vector.length)
                return { ok: false, error: "vector.upsert requires input.id and input.vector" };
        ...
        if (context.action === "vector.query") {
            ...
            if (!query && !vector.length)
                return { ok: false, error: "vector.query requires input.query or input.vector" };
