...
export class SimulationWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "simulation.schedule.run") {
            if (!projectId)
                return { ok: false, error: "simulation.schedule.run requires input.projectId" };
        ...
        if (context.action === "simulation.cost.forecast") {
            if (!projectId)
                return { ok: false, error: "simulation.cost.forecast requires input.projectId" };
        ...
        if (context.action === "digitalTwin.state.sync") {
            if (!projectId)
                return { ok: false, error: "digitalTwin.state.sync requires input.projectId or input.modelId" };
