...
export class CommerceWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "commerce.product.search") {
            ...
            if (!query)
                return { ok: false, error: "commerce.product.search requires input.query" };
        ...
        if (context.action === "commerce.order.prepare") {
            ...
            if (!item)
                return { ok: false, error: "commerce.order.prepare requires input.item or input.sku" };
        ...
        if (context.action === "payment.invoice.create") {
            ...
            if (!customer)
                return { ok: false, error: "payment.invoice.create requires input.customer" };
        ...
        if (context.action === "payment.charge.prepare") {
            ...
            if (!Number.isFinite(amount) || amount <= 0)
                return { ok: false, error: "payment.charge.prepare requires positive input.amount" };
