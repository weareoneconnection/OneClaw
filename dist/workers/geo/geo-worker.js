...
export class GeoWorker {
    ...
    async execute(input, context) {
        ...
        if (context.action === "geo.geocode") {
            ...
            if (!address)
                return { ok: false, error: "geo.geocode requires input.address" };
        ...
        if (context.action === "geo.route.plan") {
            ...
            if (!origin || !destination)
                return { ok: false, error: "geo.route.plan requires input.origin and input.destination" };
        ...
        if (context.action === "geo.site.map") {
            ...
            if (!siteId)
                return { ok: false, error: "geo.site.map requires input.siteId or input.projectId" };
